...
ax[0,0].set_ylabel(r'$\Delta$E(R)$-$$\Delta$E(R)$_{N_\mathrm{el}=720}$ (meV)', fontsize=12)
ax[0,0].set_xlim(nEl_Con[0,0],nEl_Con[-1,0])
ax[0,0].set_ylim(-16,25)
sub_ax = inset_axes(ax[0,0], width="50%", height="35%", loc=1)
...
ax[1,0].set_ylabel('Electronic States (x $10^2$)', fontsize=12)
ax[1,0].set_xlabel(r'O$-$H Distance, R ($\AA$)', fontsize=12)
ax[1,0].set_yticks([2,4,6])
ax[1,0].set_xticks([2,4,6])
# -------- Second subplot (Z) --------
...
cbar2.set_label(r'$\Delta$E(R) $-$ $\Delta$E(R)$_{N_\mathrm{el}=720}$ (meV)', fontsize=8)
cbar2.ax.tick_params(labelsize=8)
ax[1,1].set_xlabel(r'O$-$H Distance, R ($\AA$)', fontsize=12)
ax[1,1].tick_params(axis='x')
ax[1,1].tick_params( axis='y')
ax[1,1].set_yticks([2,4,6])
ax[1,1].set_xticks([2,4,6])

ax[1,0].text(0.2, 0.9, 'X Polarization', fontsize=10, transform=ax[1,0].transAxes)

ax[1,1].text(0.2, 0.9, 'Z Polarization', fontsize=10, transform=ax[1,1].transAxes)

fsize = 13
fig.text(0.03  , 0.974, f'(a)', fontsize=fsize)
fig.text(0.03, 0.48, f'(c)', fontsize=fsize)
fig.text(0.49, 0.974, f'(b)', fontsize=fsize)
fig.text(0.49, 0.48, f'(d)', fontsize=fsize)

plt.tight_layout()
plt.savefig('./Images/WaterData_fig2.png', dpi = 300, bbox_inches = 'tight')
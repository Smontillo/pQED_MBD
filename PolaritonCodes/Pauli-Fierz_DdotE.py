...
def SolvePlotandSave(H_PF,EAD,MU):
        ...
        if ( WRITE_WFNS ):
            np.savetxt( f"data_PF/U_{EVEC_OUT}_lam_{round(λ,6)}_WC_{round(wc_eV,6)}_NF_{NF}_NM_{NM}.dat", U ) # These can be large
            np.save( f"data_PF/U_{EVEC_OUT}_lam_{round(λ,6)}_WC_{round(wc_eV,6)}_NF_{NF}_NM_{NM}.dat.npy", U ) # Binary is smaller

...
def getGlobals():
    ...
    A0_LIST = [0.0,0.1] #np.arange( 0.0, 0.5+0.01, 0.01 )
    ...
    plotDIM2D = (0,2) # Plotting these two dimensions as heatmap/contour
    data_PF = "data_PF"
            ...
            for n in range( NF ):                         # Photons
                for state_j in range( NM ):               # Matter
                    ...
                    for state_k in range( NM ):           # Matter
                        ...
                        if ( state_j == 0 ): # We only have 0-->k and k-->k densities !!!!
                            DIAG_DENSITY[A0IND,a,:] += Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a] * TD_matter[state_k,:,:,:].flatten()
                        ...
                        elif ( state_k == 0 ): # We only have 0-->k and k-->k densities !!!!
                            DIAG_DENSITY[A0IND,a,:] += Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a] * TD_matter[state_j,:,:,:].flatten()
                            if ( Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a] > 1e-3 ):
                                out = "\t".join(map(str,[n,state_j,state_k,round(Upol[A0IND,polIND1,a],4), round(Upol[A0IND,polIND2,a],4), round(Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a],4)]))
                                sp.call(f"echo {out} >> data_diagonal_density/diagonal_density_contributions_{EVEC_OUT}_WC{WC}.dat",shell=True)
                                print( n,state_j,state_k,round(Upol[A0IND,polIND1,a],4), round(Upol[A0IND,polIND2,a],4), round(Upol[A0IND,polIND1,a] * Upol[A0IND,polIND2,a],4) )
...
def get_HadMU():
    ...
    for j in range( NA0 ):
        ...
        Epol[j,:] = np.loadtxt(f"{data_PF}/E_{EVEC_OUT}_lam_{round(A0,6)}_WC_{round(WC,6)}_NF_{NF}_NM_{NM}.dat")
        #Upol[j,:,:] = np.loadtxt(f"{data_PF}/U_{EVEC_OUT}_A0_{round(A0,6)}_WC_{round(WC,6)}_NF_{NF}_NM_{NM}.dat")
        Upol[j,:,:] = np.load(f"{data_PF}/U_{EVEC_OUT}_lam_{round(A0,6)}_WC_{round(WC,6)}_NF_{NF}_NM_{NM}.dat.npy")
...
def get_TD_Data():
    ...
    global NAtoms, NGrid, Nxyz, dLxyz, Lxyz, coords
    header = np.array([ np.array(j.split(),dtype=float) for j in open(f"../Density/trans-0_1.cube","r").readlines()[2:6] ])
    ...
    coords = np.array([ j for j in open(f"../Density/density.cub","r").readlines()[6:NStart] ])


    # TD = np.zeros(( NM, NM, Nxyz[0], Nxyz[1], Nxyz[2]  ))
    TD = np.zeros(( NM, Nxyz[0], Nxyz[1], Nxyz[2]  ))
    ...
    # for state_k in range(state_j,NM):
        print (f'\tReading Transition Density: {state_j}.')
        temp = []
        if state_j == 0:
            lines = open(f"../Density/density.cub",'r').readlines()[NStart:]
        else:
            lines = open(f"../Density/trans-0_{state_j}.cube",'r').readlines()[NStart:]
        ...
        if state_j == 0:
            TD[state_j,:,:,:] /= np.sum(TD[state_j,:,:,:]) * dLxyz[0]*dLxyz[1]*dLxyz[2]
            print(np.sum(TD[state_j,:,:,:]) * dLxyz[0]*dLxyz[1]*dLxyz[2])
    return TD
...
def compute_difference_density_1r( TDM_matter, diag_density ):
    ...
    # 0 (cavity) - 0 (no cavity)
    for A0IND, A0 in enumerate( A0_LIST ):
        DIFF_DENSITY[A0IND,0,:,:,:] = diag_density[A0IND,0,:,:,:] - TDM_matter[0,:,:,:]
    ...
    R2 = np.arange(Nxyz[0]) * dLxyz[0] * 0.529 # Bohr --> Angstrom
    ...
    if ( write_TD_Files == True ):
        ...
        for p in range(N_OPTIONS):
            for A0IND in range(len(A0_LIST)):
                ...
                f = open(f'data_difference_density/difference_density_{option_names[p]}_{EVEC_OUT}_A0{A0}.cube','w')
                ...
                func = np.sum( DIFF_DENSITY[A0IND,p,:,:,:] * dA * 1000, axis=axis )
            ...
            plt.tight_layout()
            plt.savefig(f'data_difference_density/difference_density.jpg',dpi=600)
            # plt.savefig(f'data_difference_density/difference_density_{option_names[p]}_{EVEC_OUT}_WC{WC}_NM{NM}_NF{NF}.jpg',dpi=600)
            plt.clf()
            np.save(f'data_difference_density/difference_density_{EVEC_OUT}.npy', np.c_[R_fine, func_interp(R_fine)])
            ...
            for A0IND,A0 in enumerate(A0_LIST):
                ...
                func_interp = interp2d(R,R2,func,kind='cubic')
                np.savetxt( f'data_difference_density/difference_density_contour_{EVEC_OUT}_{option_names[p]}_A0{A0}.dat', func_interp(RX_fine_CUT,RY_fine_CUT).T )
                np.savetxt( f'data_difference_density/difference_density_contour_RXGRID.dat', RX_fine_CUT )
                np.savetxt( f'data_difference_density/difference_density_contour_RYGRID.dat', RY_fine_CUT )

                X,Y = np.meshgrid( RX_fine_CUT,RY_fine_CUT )
                plt.contourf( X, Y,( func_interp(RX_fine_CUT,RY_fine_CUT)).T, cmap="seismic", levels=500, vmin=VMIN, vmax=VMAX)
                plt.colorbar(pad=0.01)
                plt.xlim(RX_MIN,6)#RX_MAX)
                ...
                plt.tight_layout()
                plt.savefig(f'data_difference_density/difference_density_contour_{EVEC_OUT}.jpg',dpi=600)
                plt.clf()
